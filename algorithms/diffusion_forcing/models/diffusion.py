...
class Diffusion(nn.Module):
    ...
    def _build_model(self):
        ...
        backbone = getattr(self.arch, "backbone", "unet3d")

        if len(self.x_shape) == 3:
            if backbone == "dit3d":
                h, w = self.x_shape[1], self.x_shape[2]
                ps = getattr(self.arch, "patch_size", 2)
                max_spatial = (h // ps) * (w // ps)
                self.model = EinopsWrapper(
                    from_shape="f b c h w",
                    to_shape="b c f h w",
                    module=DiT3DVideo(
                        channels=x_channel,
                        hidden_size=getattr(self.arch, "hidden_size", 384),
                        depth=getattr(self.arch, "depth", 12),
                        num_heads=getattr(self.arch, "num_heads", 6),
                        patch_size=ps,
                        mlp_ratio=getattr(self.arch, "mlp_ratio", 4.0),
                        max_frames=getattr(self.arch, "max_frames", 64),
                        max_spatial_tokens=max_spatial,
                        is_causal=self.is_causal,
                        attention_type=getattr(self.arch, "attention_type", "factorized"),
                        use_gradient_checkpointing=getattr(self.arch, "use_gradient_checkpointing", False),
                        external_cond_dim=self.external_cond_dim if self.external_cond_dim else None,
                    ),
                )
            else:
    ...
    def forward(
        ...
        zero_noise_mask: Optional[torch.Tensor] = None,
    ):
        noise = torch.randn_like(x)
        noise = torch.clamp(noise, -self.clip_noise, self.clip_noise)
        
        # Causally-Aligned Training (CAT) tryout: zero_noise_mask controls zero noise for context frames (replicating the sampling behavior for context frames)
        if zero_noise_mask is not None:
            noise = noise * (~zero_noise_mask).to(noise.dtype).reshape(
                *zero_noise_mask.shape, *([1] * (noise.ndim - zero_noise_mask.ndim))
            )
    ...
    def sample_step(
        ...
        if measurement_fn is not None and self.is_ddim_sampling:
            return self.ddim_obs_sample_step(
                x=x,
    ...
    def ddim_obs_sample_step(
        ...
        """
        DDIM step with observation guidance guidance.

        Same as ddim_sample_step but adds a post-update gradient correction:
            x_{t-1} = DDIM_step(x_t) - grad_scale * ∇_{x_t} measurement_fn(x̂_0)

        Args:
            measurement_fn: callable taking x̂_0 (predicted clean signal in
                normalized space) and returning a differentiable scalar loss.
            grad_scale: step size ζ for the observation guidance correction.
        """
        clipped_curr_noise_level = torch.where(
            curr_noise_level < 0,
            torch.full_like(curr_noise_level, self.stabilization_level - 1, dtype=torch.long),
        ...
        # inference_mode (used by Lightning's test loop) blocks enable_grad,
        # so we must exit it explicitly before enabling gradients for observation guidance.
        with torch.inference_mode(False):
            with torch.enable_grad():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    x_for_grad = x.detach().clone().float().requires_grad_(True)
                    ...
                    loss = measurement_fn(x_start)
                grad = torch.autograd.grad(loss, x_for_grad)[0]

        x_start = x_start.detach().float()
        pred_noise = pred_noise.detach().float()
        ...
        # observation guidance correction (post-update)
        x_pred = x_pred - grad_scale * grad.detach()
