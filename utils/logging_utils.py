...
def log_video(
    ...
    for i in range(n_samples):
        log_dict = {f"{namespace}/{prefix}_{i}": wandb.Video(video[i], fps=24)}
        if step is not None:
...
def get_validation_metrics_for_videos(
    ...
    :param data_range: the theoretical range of the data (max - min) used for PSNR/SSIM.
                       Default 2.0 assumes data in [-1, 1].
    ...
    # NRMSE: per-image Frobenius-norm ratio ||pred - gt||_F / ||gt||_F,
    # then averaged over all (frame, sample) pairs. The caller is expected
    # to pass only non-context frames (e.g. xs_pred[context_frames:]).
    diff_norm = torch.linalg.norm(
        (observation_hat - observation_gt).reshape(observation_hat.shape[0], -1),
        dim=1,
    )
    gt_norm = torch.linalg.norm(
        observation_gt.reshape(observation_gt.shape[0], -1),
        dim=1,
    ).clamp(min=1e-12)
    output_dict["nrmse"] = (diff_norm / gt_norm).mean()
    output_dict["psnr"] = peak_signal_noise_ratio(observation_hat, observation_gt, data_range=data_range)
    output_dict["ssim"] = structural_similarity_index_measure(observation_hat, observation_gt, data_range=data_range)
