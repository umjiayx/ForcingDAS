#!/bin/bash
# =============================================================================
# Data assimilation on SEVIR with a trained ForcingDAS model.
#
# Edit the hyperparameters below (or override via environment variables), then:
#     bash scripts/test_sevir.sh
#
...
# ── Paths / hardware ─────────────────────────────────────────────
DATA_DIR=${DATA_DIR:-/path/to/sevir}
...
CONTEXT_LENGTH=${CONTEXT_LENGTH:-6}
...
GRAD_SCALE=${GRAD_SCALE:-3.0}
GAMMA=${GAMMA:-0.01}

# ── Evaluation / logging ─────────────────────────────────────────
TEST_BATCH_SIZE=${TEST_BATCH_SIZE:-1}
LIMIT_BATCH=${LIMIT_BATCH:-4}               # null to run the full test set
WANDB_PROJECT=${WANDB_PROJECT:-forcingdas-sevir-da}
WANDB_MODE=${WANDB_MODE:-online}
TAG=${TAG:-sevir_${SCHEDULING_MATRIX}_${N_FRAMES}f_ctx${CONTEXT_LENGTH}_g${GAMMA}_gs${GRAD_SCALE}}
