#!/bin/bash
# =============================================================================
# Data assimilation on Navier-Stokes with a trained ForcingDAS model.
#
# Edit the hyperparameters below (or override via environment variables), then:
#     bash scripts/test_ns.sh
#
...
RATIO=${RATIO:-0.10}                        # kept-pixel fraction (sparse_observation)
SCALE_FACTOR=${SCALE_FACTOR:-4}             # downsample factor (super_resolution)
OP_H=${OP_H:-128}
OP_W=${OP_W:-128}
NOISE_SIGMA=${NOISE_SIGMA:-0.05}            # observation noise std
GRAD_SCALE=${GRAD_SCALE:-2.0}               # guidance step size
GAMMA=${GAMMA:-0.01}                        # noise-level-aware variance inflation (0 disables)

# ── Evaluation / logging ─────────────────────────────────────────
TEST_BATCH_SIZE=${TEST_BATCH_SIZE:-1}
LIMIT_BATCH=${LIMIT_BATCH:-4}               # null to run the full test set
WANDB_PROJECT=${WANDB_PROJECT:-forcingdas-ns-da}
WANDB_MODE=${WANDB_MODE:-online}
TAG=${TAG:-ns_${SCHEDULING_MATRIX}_${N_FRAMES}f_ctx${CONTEXT_LENGTH}_g${GAMMA}_gs${GRAD_SCALE}}
